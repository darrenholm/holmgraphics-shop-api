...
const STATUS_PATH   = process.env.FORD_TELEMATICS_STATUS_PATH   || '/v2/vehicles/:vehicleId/status';
...
async function listVehicles() {
  ...
  // Ford returns a Spring-style page: { content: [...], page: {...} }.
...
function mapStatus(status) {
  ...
  // Ford Pro's real shape (confirmed live 2026-06-18): signals are a flat OBJECT
  // keyed by name — { position, odometer, fuelLevel, ignitionState,
  // batteryVoltage, stateOfCharge, ... }, each { value/latitude/…, timestamp }.
  const sig = status.signals;
  if (sig && typeof sig === 'object' && !Array.isArray(sig)) {
    const pos = sig.position;
    if (pos) { out.lat = num(pos.latitude); out.lon = num(pos.longitude); out.location_at = pos.timestamp || null; }
    if (sig.odometer)     { out.odometer_km = num(sig.odometer.value); out.odometer_at = sig.odometer.timestamp || null; }
    if (sig.fuelLevel)      out.fuel_pct      = num(sig.fuelLevel.value);
    if (sig.ignitionState)  out.ignition      = strOrNull(sig.ignitionState.status ?? sig.ignitionState.value);
    if (sig.batteryVoltage) out.battery_volts = num(sig.batteryVoltage.value);
    if (sig.stateOfCharge)  out.ev_soc_pct    = num(sig.stateOfCharge.value);
    if (sig.batteryRange)   out.ev_range_km   = num(sig.batteryRange.value);
    else if (sig.evRange)   out.ev_range_km   = num(sig.evRange.value);
    return out;
  }

  // Shape A (fallback): array of typed signals ([{ type/signalType, value, timestamp }]).
...
async function runFordproSync({ log = console.log } = {}) {
  ...
  for (const v of vehicles) {
    // Ford's /v3/vehicles returns the id as `vehicleUuid` (used as :vehicleId
    // in the /v5/.../status path). Fall back to other names just in case.
    const vehicleId = v.vehicleUuid || v.vehicleId || v.id;

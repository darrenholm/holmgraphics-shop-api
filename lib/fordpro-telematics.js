...
//
// Unlike the consumer FordConnect wrapper (lib/fordconnect.js — Azure B2C,
// per-user authorization_code, redirect URI), this is the fleet-grade
// machine-to-machine product: a single service account using the OAuth
// *client-credentials* grant. Ford Pro bearer tokens are short-lived (~5 min),
...
//   FORD_TELEMATICS_CLIENT_ID, FORD_TELEMATICS_CLIENT_SECRET, FORD_TELEMATICS_TOKEN_URL
// Optional env:
//   FORD_TELEMATICS_BASE_URL, FORD_TELEMATICS_SCOPE, FORD_TELEMATICS_VEHICLES_PATH,
...
const TOKEN_URL = process.env.FORD_TELEMATICS_TOKEN_URL || '';   // from the Developer Portal "/Token" page
const CLIENT_ID     = process.env.FORD_TELEMATICS_CLIENT_ID     || '';
const CLIENT_SECRET = process.env.FORD_TELEMATICS_CLIENT_SECRET || '';
const SCOPE         = process.env.FORD_TELEMATICS_SCOPE || '';   // usually implicit to the service account
...
function isConfigured() {
  return Boolean(CLIENT_ID && CLIENT_SECRET && TOKEN_URL);
}
...
async function getAccessToken() {
  if (!isConfigured()) {
    throw new Error('Ford Telematics not configured (need FORD_TELEMATICS_CLIENT_ID, FORD_TELEMATICS_CLIENT_SECRET, FORD_TELEMATICS_TOKEN_URL)');
  }
  ...
  // ~5 min tokens; trust expires_in but clamp so a bad value can't pin a stale
  // token (min 60s) or hold one absurdly long (max 1h).
  const ttlMs = Math.min(Math.max(Number(json.expires_in) || 300, 60), 3600) * 1000;
  _token = { access_token: json.access_token, expires_at: Date.now() + ttlMs };

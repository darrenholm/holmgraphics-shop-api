...
// ─── HTTP wrappers ────────────────────────────────────────────────────────────
// QBO throttles per company: burst requests come back 429 ThrottleExceeded
// (errorCode 003001) with nothing written. A loop that pushes one record per
// time entry trips this easily, and a half-finished payroll sync is worse
// than a slow one — so 429 (and 5xx) is retried with a backoff rather than
// surfaced to the caller. Retry-After is honoured when QBO sends it.
const QB_RETRY_DELAYS_MS = [2000, 5000, 12000, 30000];
...
async function qbFetch(path, init) {
  ...
  for (let attempt = 0; attempt <= QB_RETRY_DELAYS_MS.length; attempt++) {
    ...
    if (!retryable || attempt === QB_RETRY_DELAYS_MS.length) return res;

    const retryAfter = parseInt(res.headers.get('retry-after') || '', 10);
    const waitMs = Number.isFinite(retryAfter)
      ? Math.min(retryAfter * 1000, 60000)
      : QB_RETRY_DELAYS_MS[attempt];
    console.warn(`QB API ${res.status} on ${path} — retrying in ${waitMs}ms`);
...
// LED ad rentals book against a dedicated "ComMarketBoard" Item so the
// income posts to its own ledger account separate from miscellaneous
// shop work. Falls back to the Misc item if the named item doesn't
// exist in QBO yet, so a stale setup doesn't take down the rental
// payment flow.
let _comMarketBoardItemId = null;
async function findComMarketBoardItemId() {
  if (_comMarketBoardItemId) return _comMarketBoardItemId;
  try {
    const data = await qbGet(
      `/query?query=${encodeURIComponent(`SELECT * FROM Item WHERE Name = 'ComMarketBoard' MAXRESULTS 1`)}`
    );
    const hit = data?.QueryResponse?.Item?.[0]?.Id;
    _comMarketBoardItemId = hit || await findMiscItemId();
  } catch {
    _comMarketBoardItemId = await findMiscItemId();
  }
  return _comMarketBoardItemId;
}
...
async function createSalesReceiptFromOrder(orderId) {
  ...
  for (const it of items) {
    ...
    // Same 6070 guard as the job-invoice path: line_subtotal is the
    // authoritative figure, unit_price is display-rounded.

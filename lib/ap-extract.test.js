...
test('parseMoneyToCents: accounting negatives', () => {
  ...
  assert.equal(parseMoneyToCents('(1,234.56)'), -123456);
  // Trailing minus, as a till prints a return. Read as positive this turned a
  // $67.80 Young's invoice into a $127.92 one.
  assert.equal(parseMoneyToCents('60.12-'),     -6012);
  assert.equal(parseMoneyToCents('1,234.56-'),  -123456);
});
...
// ─── linesReconcile ──────────────────────────────────────────────────────
// The Excel Business Systems invoice showed "Contract copies charge $78.65"
// and then broke the same charge out as two meter readings ($55.47 +
// $23.18 = $78.65). Both were captured, and the bill posted to QuickBooks
// at exactly double: $177.74 against a document reading $88.87.

const { linesReconcile } = require('./ap-extract');

const cents = (...amounts) => amounts.map((a) => ({ amount_cents: a }));

test('linesReconcile: lines plus tax reaching the total is fine', () => {
  const r = linesReconcile({ lines: cents(7865), taxCents: 1022, totalCents: 8887 });
  assert.equal(r.ok, true);
  assert.equal(r.checked, true);
  assert.equal(r.lineTotalCents, 7865);
  assert.equal(r.differenceCents, 0);
});

test('linesReconcile: catches the doubled Excel invoice', () => {
  // summary 78.65 + its own breakdown 55.47 + 23.18, all captured
  const r = linesReconcile({ lines: cents(0, 7865, 5547, 2318), taxCents: 1022, totalCents: 8887 });
  assert.equal(r.ok, false);
  assert.equal(r.lineTotalCents, 15730);
  assert.equal(r.expectedCents, 16752);
  assert.equal(r.differenceCents, 16752 - 8887);
});

test('linesReconcile: a one-cent rounding difference passes', () => {
  assert.equal(linesReconcile({ lines: cents(7865), taxCents: 1022, totalCents: 8888 }).ok, true);
  assert.equal(linesReconcile({ lines: cents(7865), taxCents: 1022, totalCents: 8886 }).ok, true);
  assert.equal(linesReconcile({ lines: cents(7865), taxCents: 1022, totalCents: 8889 }).ok, false);
});

test('linesReconcile: a missing tax amount counts as zero tax', () => {
  assert.equal(linesReconcile({ lines: cents(5000), taxCents: null, totalCents: 5000 }).ok, true);
  assert.equal(linesReconcile({ lines: cents(5000), taxCents: null, totalCents: 5650 }).ok, false);
});

test('linesReconcile: credit notes reconcile the same way', () => {
  const r = linesReconcile({ lines: cents(-3982), taxCents: -518, totalCents: -4500 });
  assert.equal(r.ok, true);
});

// Nothing to compare against is not a failure — criticalFieldsMissing() is
// what flags an invoice with no total. Reporting "does not add up" here
// would block every document whose total was simply unreadable.
test('linesReconcile: no total or no lines means nothing to check', () => {
  const noTotal = linesReconcile({ lines: cents(5000), taxCents: 0, totalCents: null });
  assert.equal(noTotal.ok, true);
  assert.equal(noTotal.checked, false);

  const noLines = linesReconcile({ lines: [], taxCents: 0, totalCents: 5000 });
  assert.equal(noLines.ok, true);
  assert.equal(noLines.checked, false);
});

// A column left out of a SELECT arrives as undefined. Read as zero tax, that
// refuses every taxable invoice by exactly its own tax amount — which is
// what the approve route did until its SELECT was fixed. null (no tax on the
// document) and undefined (nobody supplied it) must not behave alike.
test('linesReconcile: an unsupplied tax is skipped, not treated as zero', () => {
  const missing = linesReconcile({ lines: cents(24633), totalCents: 27835 });
  assert.equal(missing.ok, true, 'must not fail an invoice merely because tax was not passed');
  assert.equal(missing.checked, false);

  // Explicit null still means zero tax, and still gets checked.
  const zeroTax = linesReconcile({ lines: cents(24633), taxCents: null, totalCents: 27835 });
  assert.equal(zeroTax.ok, false);
  assert.equal(zeroTax.checked, true);
});

// ─── Bundles ─────────────────────────────────────────────────────────────
// A bad page range invents a bill or cuts one in half, so anything that is
// not a clean ascending run is discarded and the file is read as one document.

const { normalizeDocumentPages } = require('./ap-extract')._internals;

test('document_pages: a clean run of invoices is kept', () => {
  assert.deepEqual(
...
test('document_pages: one document is not a bundle', () => {
  assert.deepEqual(normalizeDocumentPages([{ doc_number: 'A', first_page: 1, last_page: 4 }]), []);
  assert.deepEqual(normalizeDocumentPages([]), []);
  assert.deepEqual(normalizeDocumentPages(null), []);
});

test('document_pages: overlapping ranges are thrown away, not trusted', () => {
  assert.deepEqual(
    normalizeDocumentPages([
      { doc_number: 'A', first_page: 1, last_page: 3 },
...
test('document_pages: out of order, backwards or fractional ranges are thrown away', () => {
  const bad = [
    [{ first_page: 3, last_page: 4 }, { first_page: 1, last_page: 2 }],
    [{ first_page: 1, last_page: 2 }, { first_page: 4, last_page: 3 }],
    [{ first_page: 0, last_page: 2 }, { first_page: 3, last_page: 4 }],
    [{ first_page: 1.5, last_page: 2 }, { first_page: 3, last_page: 4 }],
  ];
  for (const ranges of bad) assert.deepEqual(normalizeDocumentPages(ranges), []);
});

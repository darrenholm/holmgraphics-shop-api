...
test('ten car door decals nest two across and cost the roll they consume', () => {
  const line = priceDecals({ widthIn: 20, heightIn: 12, quantity: 10 });
  // 2 across, 5 rows, 64" of roll => 24 sq ft at $12
  assert.equal(line.total, 288.00);
  assert.equal(line.item_type, 'Printing');
});
...
test('artwork is charged once for the job, not once per item', () => {
  ...
  assert.equal(order.subtotal, 210.00 + 110.70 + 288.00 + ARTWORK_FEE);
});

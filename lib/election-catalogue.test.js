...
test('stands are counted separately from signs — poles and fences need none', () => {
  // 24 signs, 10 stands. Charging a stand per sign would overcharge every
  // campaign that puts some on utility poles, which is most of them.
  const line = priceSigns({ cutKey: '12x16', sheetKey: '4mm-single', quantity: 24, stands: 10 });
  assert.equal(line.stands, 10);
  assert.equal(line.total, 210.00 + 10 * 2.10);
  assert.match(line.description, /with 10 wire stands/);
});

test('no stands asked for, none charged', () => {
  const line = priceSigns({ cutKey: '12x16', sheetKey: '4mm-single', quantity: 24 });
  assert.equal(line.stands, 0);
  assert.equal(line.total, 210.00);
  assert.doesNotMatch(line.description, /stand/);
});

test('one stand reads as one, not "1 wire stands"', () => {
  const line = priceSigns({ cutKey: '12x16', sheetKey: '4mm-single', quantity: 24, stands: 1 });
  assert.match(line.description, /with 1 wire stand,|with 1 wire stand$/);
});

test('more stands than signs is allowed — campaigns buy spares', () => {
  const line = priceSigns({ cutKey: '12x16', sheetKey: '4mm-single', quantity: 24, stands: 30 });
  assert.equal(line.stands, 30);
  assert.equal(line.total, 210.00 + 30 * 2.10);
});

test('a draft saved when stands was a checkbox still means one per sign', () => {
  const line = priceSigns({ cutKey: '12x16', sheetKey: '4mm-single', quantity: 24, stands: true });
  assert.equal(line.stands, 24);
  assert.equal(line.total, 210.00 + 24 * 2.10);
});

test('a size no stand fits is never charged for one', () => {
  // A 32x48 goes on posts. Asking for stands does not add a charge for
  // something the shop would not supply.
  const big = priceSigns({ cutKey: '32x48', sheetKey: '4mm-single', quantity: 2, stands: 6 });
  assert.equal(big.stands, 0);
  assert.equal(big.total, 210.00);
  assert.ok(big.mounting, 'a large sign carries a mounting note');
});

test('every cut yields what actually fits on a 4 x 8 sheet', () => {
  // The yield is not a judgement call — it is what a guillotine gets out of a
  // 48" x 96" sheet, whichever way round the sign is laid. A 32 x 48 was once
  // listed as 2 a sheet when three fit end to end, and every quote for that
  // size was half a sheet too dear.
  const fits = (w, h) =>
    Math.max(
      Math.floor(48 / w) * Math.floor(96 / h),
      Math.floor(48 / h) * Math.floor(96 / w)
    );

  for (const cut of SIGN_CUTS) {
    const [w, h] = cut.key.split('x').map(Number);
    assert.equal(cut.perSheet, fits(w, h), `${cut.name} per sheet`);
  }
});

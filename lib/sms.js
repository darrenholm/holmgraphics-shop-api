...
async function cloudmessageSend({ to, body }) {
  ...
  const res = await fetch(CM.sendUrl, {
    ...
    headers: {
      ...
      'Content-Type': 'application/json',
      // Required by CloudMessage/Textable — without it the API rejects the
      // request (as a 403 "Invalid API Credentials"). Confirmed against a
      // working curl from SkySwitch support.

...
test('credit row on an invoice number is money on account, not a mismatch', () => {
  const { lines, summary } = matchStatementLines(
    [stmtLine(1, '7330122', -31995, 'credit')],
    [bill(10, '7330122', 2461.14)],
    []
  );
  assert.equal(lines[0].match_status, 'ignored');
  assert.equal(summary.amount_mismatch, 0);
  // Still part of what the statement says we owe.
  assert.equal(summary.statement_charges_cents, -31995);
});

test('credit row matching a vendor credit is still reconciled normally', () => {
  const { lines } = matchStatementLines(
    [stmtLine(1, 'CM-118', -4500, 'credit')],
    [bill(10, 'CM-118', -45.00)],
    []
  );
  assert.equal(lines[0].match_status, 'matched');
});

...
function apparelRetail(wholesale) {
  // NULL IS NOT ZERO, AND ZERO IS NOT A PRICE. Number(null) is 0 and
  // Number('') is 0, both of which are finite — so a variant with no wholesale
  // loaded sailed through the "under $5 is $10 flat" rule and came out at $10.
  // Every colour and every size of the ATC1000 quoted $10, 6XL included, which
  // is what sending this live would have charged for them.
  if (wholesale === null || wholesale === undefined || wholesale === '') return null;
...
async function apparelOptions() {
  ...
  // The cheapest printed garment of each style, for the "from" on a card. The
  // garment alone would be a lie by omission — nobody orders an undecorated
  // campaign shirt — so the cheapest decoration goes in with it.
  const { printLocationPrices } = await getConfig();
  const cheapestPrint = Math.min(
    ...(printLocationPrices || []).map((p) => Number(p.price_per_piece)),
  );
  ...
  return ELECTION_STYLES
    ...
    .filter(Boolean)
    .map((entry) => {
      const colours = [...entry.colours.values()];
      const garment = Math.min(...colours.flatMap((c) => c.sizes.map((s) => s.price)));
      const from = Number.isFinite(cheapestPrint) ? garment + cheapestPrint : garment;
      return { ...entry, colours, from: Math.round(from * 100) / 100 };
    });
}

...
async function splitByRanges(buffer, ranges) {
  ...
  // EVERY page must belong to exactly one invoice. Ranges that merely fit
  // inside the file are not enough: Timbremart's seven-page reprint pack came
  // back as three ranges covering the first few pages, and splitting on that
  // produced three bills and silently dropped four — the parent is marked
  // done and drops out of the queue, so nothing downstream ever asks where
  // they went. That is the exact failure this whole feature exists to fix, so
  // a gap at either end or in the middle refuses the split outright.
  if (ranges[0].first_page !== 1 || ranges[ranges.length - 1].last_page !== total) {
    throw new Error(
      `Split ranges cover pages ${ranges[0].first_page}-${ranges[ranges.length - 1].last_page} ` +
      `of a ${total}-page file; every page must belong to an invoice`
    );
  }
  for (let i = 1; i < ranges.length; i++) {
    if (ranges[i].first_page !== ranges[i - 1].last_page + 1) {
      throw new Error(
        `Split ranges leave a gap between pages ${ranges[i - 1].last_page} and ${ranges[i].first_page}`
      );
    }
  }

...
// sendJobMessageEmail({ projectId, message, db, senderName })
//   Manual, staff-composed email to whoever is currently assigned to the job.
//   Same shape as sendJobMessageSms but over Resend (which delivers today,
//   unlike the SkySwitch outbound SMS path). Repeatable, audit = the job
//   note the route writes. Never throws.
//
// Returns { sent: bool, to?, employee_id?, employee_name?, reason? }.
async function sendJobMessageEmail({ projectId, message, db, senderName = null }) {
  if (projectId == null) return { sent: false, reason: 'no_project' };
  const text = typeof message === 'string' ? message.trim() : '';
  if (!text) return { sent: false, reason: 'empty_message' };
  ...
  if (!email) {
    return { sent: false, reason: 'no_email', employee_id: row.employee_id, employee_name: employeeName };
  }

  let result;
  try {
    result = await mailer.sendJobMessageToEmployee({
      email,
      employeeName,
      projectId: row.project_id,
      ...
      message: text,
      senderName,
      jobUrl: `${STAFF_BASE}/jobs/${row.project_id}`,
    });
  } catch (err) {
    console.warn(`[employee-notifier] email send failed for project ${projectId}:`, err.message);
    return { sent: false, reason: 'send_failed', error: err.message, employee_id: row.employee_id, employee_name: employeeName };
  }

  return result?.ok
    ? { sent: true, to: email, employee_id: row.employee_id, employee_name: employeeName }
    : { sent: false, reason: 'send_failed', error: result?.error, employee_id: row.employee_id, employee_name: employeeName };
}

module.exports = { sendJobAssignedSms, sendJobMessageSms, sendJobMessageEmail, _internals: { KIND, MESSAGE_KIND, buildBody, formatDue } };

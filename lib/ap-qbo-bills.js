...
const { normalizeVendor, linesReconcile } = require('./ap-extract');

// Ontario HST, same code the invoice and salesreceipt paths use.
const HST_TAX_CODE = process.env.QBO_AP_TAX_CODE || '7';

// The rate HST_TAX_CODE actually stands for, as a percent. Every line the
// intake writes is coded HST, so a supplier who bills at some other rate
// would have the difference invented on their behalf — see taxRateMismatch.
// Configurable only so a company file on a different code doesn't need a
// deploy to correct it.
const AP_TAX_RATE_PERCENT = Number(process.env.QBO_AP_TAX_RATE_PERCENT || 13);

// Rounding room between the supplier's printed tax and that rate applied to
// their own line total. A multi-line invoice rounds per line, so a couple of
// cents is normal; twenty-six dollars is a different rate.
const TAX_RATE_SLACK_CENTS = 5;
...
// Every expense account in the chart of accounts, for the per-line coding
// dropdown. Classification rather than AccountType so Cost of Goods Sold and
// Other Expense come along too — freight and materials usually live in one of
// those, not in plain "Expense".
//
// Cached: the chart of accounts changes a few times a year, and this is hit
// once per review screen.
async function listExpenseAccounts() {
  return cachedLookup('expense-accounts', async () => {
    const out = [];
    const PAGE = 200;
    for (let start = 1; ; start += PAGE) {
      const sql =
        `SELECT Id, Name, AccountType, AccountSubType, FullyQualifiedName FROM Account ` +
        `WHERE Classification = 'Expense' AND Active = true ` +
        `STARTPOSITION ${start} MAXRESULTS ${PAGE}`;
      const data = await qbCall(
        () => qbGet(`/query?query=${encodeURIComponent(sql)}`),
        { label: 'listExpenseAccounts' }
      );
    ...
    return out.map((a) => ({
      id:       a.Id,
      // FullyQualifiedName carries the parent, so two accounts both called
      // "Freight" under different parents stay tellable apart.
...
// The account this vendor's lines were coded to most often. Freight vendors
// always go to the same place, so once Little Rock Farms has been coded to
// Shipping, every later invoice from them arrives already coded — the same
// learn-once idea as the vendor alias, without anyone having to say "remember
// this".
async function suggestAccountForVendor(vendorQboId) {
  if (!vendorQboId) return null;
  const row = await queryOne(
    `SELECT l.account_qbo_id, l.account_name, COUNT(*) AS uses
       FROM ap_document_lines l
       JOIN ap_documents d ON d.id = l.document_id
      WHERE d.vendor_qbo_id = $1
        AND l.account_qbo_id IS NOT NULL
        AND d.review_status = 'approved'
      GROUP BY l.account_qbo_id, l.account_name
      ORDER BY uses DESC
      LIMIT 1`,
    [vendorQboId]
  );
  if (!row) return null;
  return { id: row.account_qbo_id, name: row.account_name };
}
...
async function listBillsForVendor({ vendorQboId, since, until }) {
  ...
  return pageVendorTxns('Bill', clauses, vendorQboId);
}

/**
 * Vendor credits for the same vendor and window.
 *
 * A credit note entered in QuickBooks is a VendorCredit, not a Bill, so a
 * reconciliation that queried only Bill reported every credit on the statement
 * as missing however carefully it had been entered — which trains the reviewer
 * to ignore the one finding that matters.
 *
 * Returned with TotalAmt NEGATED, because that is how the statement prints a
 * credit and the matcher compares the two directly.
 */
async function listVendorCreditsForVendor({ vendorQboId, since, until }) {
  if (!vendorQboId) return [];
  ...
  const rows = await pageVendorTxns('VendorCredit', clauses, vendorQboId);
  return rows.map((r) => ({
    ...r,
    TotalAmt: -Math.abs(Number(r.TotalAmt || 0)),
    // VendorCredit has no Balance; nothing is outstanding on a credit.
    Balance: 0,
    _isVendorCredit: true,
  }));
}

async function pageVendorTxns(entity, clauses, vendorQboId) {
  const out = [];
  ...
  const PAGE = 500;
  for (let start = 1; ; start += PAGE) {
    const columns = entity === 'VendorCredit'
      ? 'Id, DocNumber, TxnDate, TotalAmt'
...
function buildBillPayload({ doc, lines, defaultAccountId, termId }) {
  ...
  if (doc.tax_cents !== null && doc.tax_cents !== undefined) {
    ...
    // A Canadian company file has to be TOLD how tax applies. Without this
    // QBO falls back to 'NotApplicable', the bill reads "Amounts are: Out of
    // Scope of Tax", the tax is dropped and the input tax credit is silently
    // forfeited — the whole point of coding the line. Supplier invoices print
    // a subtotal with tax added underneath, so what we send is tax-EXCLUSIVE.
    // A genuinely untaxed bill says so outright rather than leaving QBO to
    // fall back to the same value by accident.
    payload.GlobalTaxCalculation = doc.tax_cents > 0 ? 'TaxExcluded' : 'NotApplicable';
  }
...
/**
 * Why a bill's printed tax doesn't match the rate its lines are coded at.
 *
 * Intake codes every taxed line HST, because Canadian supplier invoices are
 * 13% far more often than not. With GlobalTaxCalculation now set, QBO acts on
 * that code — so an invoice the supplier taxed at 5% would be booked at 13%
 * and claim back tax nobody charged. That is the one error in this path with
 * a consequence at the CRA rather than in a reconciliation, so it stops the
 * post instead of warning after the bill is already real.
 *
 * Returns null when the rate is right, or a message explaining the mismatch.
 */
function taxRateMismatch({ lines, taxCents }) {
  if (!taxCents || taxCents <= 0) return null;
  if (!lines.some((l) => l.tax_code)) return null;

  const lineTotal = lines.reduce((sum, l) => sum + (l.amount_cents || 0), 0);
  if (lineTotal <= 0) return null;

  const expected = Math.round(lineTotal * (AP_TAX_RATE_PERCENT / 100));
  if (Math.abs(taxCents - expected) <= TAX_RATE_SLACK_CENTS) return null;

  const fmt = (c) => `$${(c / 100).toFixed(2)}`;
  const actualPercent = ((taxCents / lineTotal) * 100).toFixed(2);
  return (
    `the supplier taxed ${fmt(lineTotal)} at ${actualPercent}% (${fmt(taxCents)}), but every ` +
    `line is coded at ${AP_TAX_RATE_PERCENT}%, which would book ${fmt(expected)}. Posting it ` +
    `would claim back tax that was never charged. Set the right tax code on the lines, or ` +
    `check whether the supplier billed the wrong rate.`
  );
}
...
async function postBillForDocument(documentId) {
  ...
  // Backstop for the same check the approve route makes. Approval can be
  // granted and the document edited afterwards, and this function is also
  // reachable directly, so the arithmetic is verified here too rather than
  // trusted from upstream. Refusing costs a re-check; posting a doubled bill
  // costs a wrong payable that reconciles against nothing.
  ...
  if (!sums.ok) {
    const fmt = (c) => `$${(c / 100).toFixed(2)}`;
    throw new Error(
      `Refusing to post document ${documentId}: lines total ${fmt(sums.lineTotalCents)} ` +
      `plus tax ${fmt(doc.tax_cents || 0)} = ${fmt(sums.expectedCents)}, but the document ` +
      `says ${fmt(doc.total_cents)}. Fix the lines first — a charge is probably counted twice.`
    );
  }

  const rateProblem = taxRateMismatch({ lines, taxCents: doc.tax_cents });
  if (rateProblem) {
    throw new Error(`Refusing to post document ${documentId}: ${rateProblem}`);
  }
...
module.exports = {
  ...
  attachPdfToBill,
  listBillsForVendor,
  listVendorCreditsForVendor,
  findExistingBill,
  findVendorByName,
  searchVendors,
  listExpenseAccounts,
  suggestAccountForVendor,
  findAccountByName,
  findTermByName,
  clearRefCache,
  HST_TAX_CODE,
  // Exported for unit tests.
  _internals: { buildBillPayload, centsToDollars, qbqlEscape, qbCall, taxRateMismatch },
};

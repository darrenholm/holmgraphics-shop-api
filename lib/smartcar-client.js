...
function isConfigured() {
  return !!(getApplicationId() && getClientId() && process.env.SMARTCAR_CLIENT_SECRET && process.env.SMARTCAR_REDIRECT_URI);
}

// Smartcar Connect 2.0 uses TWO identifiers per application:
//
//   application_id  — UUID (e.g. 786bd657-8e61-44c8-938c-d2ba1048fb55)
//                     visible in the dashboard URL: /apps/<UUID>/...
//                     used in the OAuth /authorize URL as ?application_id=
//
//   client_id       — `client_<ULID>` (e.g. client_01KS5JTX9C6RZM3GG5VTAFJ5CR)
//                     visible on the Configuration page as "Client ID"
//                     used in the /oauth/token exchange (basic auth) along
//                     with client_secret
//
// If you mix them up (use the UUID for the token exchange), Smartcar
// returns `invalid_client - Invalid or incorrect client credentials` on
// the callback's exchangeCode call. Symptom: user authorizes Ford/etc
// successfully but lands back without a Smartcar link created.
//
// Env vars:
//   SMARTCAR_APPLICATION_ID  — the UUID (required for new apps)
//   SMARTCAR_CLIENT_ID       — the `client_<ULID>` value
//   SMARTCAR_CLIENT_SECRET   — secret paired with client_id
//
// Backward compat: if SMARTCAR_APPLICATION_ID isn't set, fall back to
// SMARTCAR_CLIENT_ID for the application_id (works only for legacy apps
// whose client_id IS the UUID).
function getApplicationId() {
  return process.env.SMARTCAR_APPLICATION_ID || process.env.SMARTCAR_CLIENT_ID;
}
function getClientId() {
  return process.env.SMARTCAR_CLIENT_ID;
}
...
function authClient() {
  ...
  if (!_authClient) {
    _authClient = new smartcar.AuthClient({
      clientId:     getClientId(),                  // client_<ULID> — used by exchangeCode + refresh
...
function getAuthUrl({ userId, vehicleId }) {
  if (!isConfigured()) {
    const e = new Error('Smartcar is not configured.');
    e.code = 'SMARTCAR_NOT_CONFIGURED';
    throw e;
  }
  const state = signState({ userId, vehicleId });

  // Smartcar Connect 2.0 (current as of 2026) uses the `application_id`
  // query param against connect.smartcar.com/oauth/authorize. The legacy
  // SDK helper still emits `client_id`, which the new endpoint rejects
  // with "Invalid parameter client_id". We build the URL manually so the
  // param names match what Smartcar's own "Share Connect link" generates.
  //
  // Reference URL from the dashboard's Share-Connect-Link feature:
  //   https://connect.smartcar.com/oauth/authorize
  //     ?response_type=code
  //     &application_id=<UUID-from-dashboard-URL>
  //     &mode=live | simulated
  //
  // We additionally append redirect_uri, scope, state, approval_prompt
  // so the OAuth code can flow back into our callback handler.
  const params = new URLSearchParams({
    response_type:   'code',
    application_id:  getApplicationId(),            // UUID — used by the authorize URL only
    redirect_uri:    process.env.SMARTCAR_REDIRECT_URI,
    scope:           SCOPES.join(' '),
    state,
    mode:            getMode(),
    approval_prompt: 'force'
  });
  const url = `https://connect.smartcar.com/oauth/authorize?${params.toString()}`;
  return { url, state };
}

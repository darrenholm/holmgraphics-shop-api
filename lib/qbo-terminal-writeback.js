...
// QBO id anywhere, so DocNumber is the only link back.

// Every invoice carrying this job's number, settled or not.
async function findInvoicesForProject(projectId) {
  if (!projectId) return [];
  const data = await qbCall(
    () => qbGet(`/query?query=${encodeURIComponent(
      `SELECT Id, DocNumber, Balance, TotalAmt, TxnDate, DueDate, CustomerRef FROM Invoice ` +
...
// Balance matters for the write-back: an already-settled invoice can't take
// another payment without turning into a customer credit, so a zero-balance
// hit is treated as "no invoice".
async function findOpenInvoiceForProject(projectId) {
  const invoices = await findInvoicesForProject(projectId);
  return invoices.find((inv) => Number(inv.Balance) > 0) || null;
}

// What the counter needs to know when someone walks in holding an invoice:
// what's actually outstanding on it right now, which is not necessarily what
// the job's line items add up to. Never throws — a QuickBooks outage must not
// stop anyone taking a payment, it just means falling back to the job total.
async function invoiceSummaryForProject(projectId) {
  try {
    const invoices = await findInvoicesForProject(projectId);
    if (!invoices.length) return { found: false };
    const open = invoices.find((inv) => Number(inv.Balance) > 0);
    const inv  = open || invoices[0];
    return {
      found:        true,
      invoiceId:    inv.Id,
      docNumber:    inv.DocNumber,
      totalCents:   dollarsToCents(inv.TotalAmt),
      balanceCents: dollarsToCents(inv.Balance),
      settled:      !open,
...
module.exports = {
  ...
  findInvoicesForProject,
  invoiceSummaryForProject,

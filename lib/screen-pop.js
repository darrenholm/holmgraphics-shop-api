...
// The same card, but for a client we already know the id of. Used after a
// staffer links an unrecognised number to a customer mid-call: the pop swaps
// from "no matching client" to their real card without them having to hang up
// and go looking for it.
async function cardForClientId(clientId) {
  const rows = await query(
    `SELECT c.id,
            COALESCE(c.company, CONCAT_WS(' ', c.fname, c.lname)) AS name,
            c.company,

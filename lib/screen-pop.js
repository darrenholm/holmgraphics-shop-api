...
// Open jobs for one client, newest first, with the same per-job value
// definition the job board uses (manual line items + any linked online order).
  ...
  return query(
    `SELECT p.id,
       ...
       LEFT JOIN (SELECT job_id,     SUM(grand_total) AS total FROM orders GROUP BY job_id)     ov
              ON ov.job_id     = p.id
      WHERE p.client_id = $1
        AND p.status_id BETWEEN $2 AND $3
      ORDER BY p.created_date DESC NULLS LAST, p.id DESC`,
    [clientId, ACTIVE_STATUS_MIN, ACTIVE_STATUS_MAX]
  );
}
...
async function clientCard(client) {
  ...
  // Oldest open job drives the "waiting since" line — the useful signal when
  // someone rings up asking where their sign is.
  const oldest = jobs.reduce((acc, j) => {

...
async function runExtraction(documentId) {
  ...
  if (result.document_pages?.length >= 2) {
    ...
    // Falling through means the split could not be trusted. The document is
    // read as the single invoice the rest of this function assumes, and the
    // reviewer sees a bill whose lines do not add up to its total — which is
    // the existing warning, and a great deal better than silence.
  }

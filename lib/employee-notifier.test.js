...
const { sendJobMessageSms, sendJobMessageEmail, _internals } = require('./employee-notifier');
...
// ─── sendJobMessageEmail (the job-page "Email <assignee>" button) ────────────
// The mailer runs in stub mode here (no RESEND_API_KEY in tests), so sends
// "succeed" without the network — locking in that stub sends count as sent.
...
const EMAIL_ROW = {
  ...
  first_name: 'Brady',
  last_name: 'Holm',
  email: 'brady@holmgraphics.ca',
};

test('sendJobMessageEmail: happy path sends to the assignee address', async () => {
  const db = fakeDb(EMAIL_ROW);
  const r = await sendJobMessageEmail({ projectId: 123, message: 'Rush this one', db, senderName: 'Darren Holm' });
  assert.equal(r.sent, true);
  assert.equal(r.to, 'brady@holmgraphics.ca');
  assert.equal(r.employee_id, 12);
  assert.equal(r.employee_name, 'Brady Holm');
});

test('sendJobMessageEmail: empty message rejected before any lookup', async () => {
  const r = await sendJobMessageEmail({ projectId: 123, message: '   ', db: fakeDb(EMAIL_ROW) });
  assert.equal(r.sent, false);
  assert.equal(r.reason, 'empty_message');
});

test('sendJobMessageEmail: no assignee reports no_assignee', async () => {
  const r = await sendJobMessageEmail({ projectId: 123, message: 'hello', db: fakeDb({ ...EMAIL_ROW, employee_id: null }) });
  assert.equal(r.sent, false);
  assert.equal(r.reason, 'no_assignee');
});

test('sendJobMessageEmail: assignee without email reports no_email with the name', async () => {
  for (const bad of [null, '', '   ']) {
    const r = await sendJobMessageEmail({ projectId: 123, message: 'hello', db: fakeDb({ ...EMAIL_ROW, email: bad }) });
    assert.equal(r.sent, false);
...
test('sendJobMessageEmail: missing project reports project_not_found', async () => {
  const r = await sendJobMessageEmail({ projectId: 999, message: 'hello', db: fakeDb(null) });
  assert.equal(r.sent, false);
  assert.equal(r.reason, 'project_not_found');
});

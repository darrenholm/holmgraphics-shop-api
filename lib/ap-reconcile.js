...
function matchStatementLines(statementLines, ourBills, ourDocuments, opts = {}) {
  ...
  for (const sl of statementLines || []) {
    ...
    // A credit row carrying an INVOICE's number is money the supplier holds on
    // account against that invoice (an overpayment), not a credit memo — a
    // memo has its own number and matches a VendorCredit, which arrives here
    // negative. Comparing it to the invoice total reports a false mismatch
    // every month until the money is used.
    const heldOnAccount = base.kind === 'credit' && (sl.amount_cents ?? 0) < 0 &&
      candidates.length > 0 && candidates.every((b) => Number(b.TotalAmt || 0) > 0 &&
        !amountsAgree(sl.amount_cents, Math.round(Number(b.TotalAmt || 0) * 100), toleranceCents));
    if (heldOnAccount) {
      lines.push({
        ...base,
        match_status:        'ignored',
        matched_qbo_bill_id: String(candidates[0].Id),
        note: 'Credit held on account against this invoice (an overpayment), not a credit memo',
      });
      continue;
    }

...
const { buildBillPayload, centsToDollars, taxRateMismatch } = _internals;
...
// Found on a real SanMar bill (QBO bill 32115, doc /admin/ap/30): the bill
// showed "Amounts are: Out of Scope of Tax" with no tax at all, because a
// Canadian company file falls back to NotApplicable unless the payload says
// otherwise. Every bill this pipeline had posted was silently forfeiting the
// input tax credit.

test('tax: a taxed bill declares itself tax-exclusive', () => {
  const p = buildBillPayload({ doc: doc(), lines: [line()], defaultAccountId: '63' });
  assert.equal(p.GlobalTaxCalculation, 'TaxExcluded');
});

test('tax: a zero-tax bill says not-applicable rather than leaving it unset', () => {
  const p = buildBillPayload({
    doc: doc({ tax_cents: 0, total_cents: 100000 }),
    lines: [line({ tax_code: null })],
    defaultAccountId: '63',
...
test('tax: an unread tax amount asserts no treatment at all', () => {
  const p = buildBillPayload({
    doc: doc({ tax_cents: null }),
    lines: [line()],
    defaultAccountId: '63',
  });
  assert.equal(p.GlobalTaxCalculation, undefined);
});

// ─── taxRateMismatch ────────────────────────────────────────────────────────
// Both fixtures are real SanMar Canada invoices. One is billed at 13% and must
// post; the other at 5% and must not, because intake codes every line HST and
// posting it would claim an input tax credit for $26.40 nobody charged.

test('rate: SanMar at 13% posts (607.50 + 78.98)', () => {
  assert.equal(
    taxRateMismatch({ lines: [line({ amount_cents: 60750 })], taxCents: 7898 }),
    null,
  );
});

test('rate: SanMar at 5% is refused (330.00 + 16.50)', () => {
  const msg = taxRateMismatch({ lines: [line({ amount_cents: 33000 })], taxCents: 1650 });
  assert.ok(msg, 'expected a mismatch message');
  assert.ok(msg.includes('5.00%'), msg);
  assert.ok(msg.includes('$42.90'), msg);   // what coding it HST would have booked
  assert.ok(msg.includes('never charged'), msg);
});

test('rate: per-line rounding on a multi-line invoice is not a mismatch', () => {
  const lines = [
    line({ line_no: 1, amount_cents: 1250 }),
    line({ line_no: 2, amount_cents: 1250 }),
    line({ line_no: 3, amount_cents: 1250 }),
  ];
  const total = 1233 + 4567 + 8901;
  // Each line rounded on its own, which is how suppliers print it.
  const perLine = lines.reduce((s, l) => s + Math.round(l.amount_cents * 0.13), 0);
  assert.notEqual(perLine, Math.round(total * 0.13), 'fixture should actually differ');
  assert.equal(taxRateMismatch({ lines, taxCents: perLine }), null);
});

test('rate: uncoded lines are left alone — nothing is being claimed', () => {
  assert.equal(
    taxRateMismatch({ lines: [line({ tax_code: null, amount_cents: 33000 })], taxCents: 1650 }),
    null,
  );
});

test('rate: a bill with no tax is not checked', () => {
  assert.equal(taxRateMismatch({ lines: [line()], taxCents: 0 }), null);
  assert.equal(taxRateMismatch({ lines: [line()], taxCents: null }), null);
});

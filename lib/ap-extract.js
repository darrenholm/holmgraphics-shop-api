...
const { pageCount } = require('./ap-split');
...
const StatementLineSchema = z.object({
  ...
  finance_charge: z.string(),
});

const DocumentSchema = z.object({
  doc_kind:   z.enum(['invoice', 'statement', 'credit_note', 'payment', 'unknown']),
  ...
  // Non-empty only when one PDF holds several separate invoices.
  document_pages: z.array(z.object({
    doc_number: z.string(),
    first_page: z.number(),
    last_page:  z.number(),
  })),
...
- "payment" — a receipt for money paid against an account rather than a charge: "house account payment", "payment on account", "thank you for your payment", a counter receipt for an EFT or cheque. It carries an amount and usually no line items and no tax. It is not a bill; recording it as one would pay the supplier twice.
...
Returns and credits sitting inside an ordinary invoice are lines like any other, and must be recorded as NEGATIVE amounts. A returned item, a goodwill credit or a restocking refund is often printed with a trailing minus ("60.12-"), in parentheses, marked CR, or with a negative quantity ("4-"), sometimes with a reason underneath it such as "Return Reason: Non-Soiled". Copy the amount as printed, minus sign and all. Dropping such a line, or recording it as a positive, overstates the bill by twice the credit.

CRITICAL — never record the same charge twice. Many invoices state a charge once as a summary and again broken into its parts, often under headings like "Summary" and "Detail", or as a service line followed by the meter readings or item lines that make it up. Capture ONE level only. Prefer the itemized detail when its parts add up to the summary figure; otherwise take the summary. A charge counted at both levels doubles the bill, and a doubled bill looks entirely plausible in the accounts.

Before you answer, add your line amounts together, add the tax, and check the result equals the total printed on the document. If it does not, you have captured a charge twice, missed one, or picked up a summary row as a line — fix it rather than returning figures that do not add up.
...
- amount is the balance the statement shows for that row, as printed.
- finance_charge is interest the supplier has added to that row, where the statement prints it as its own figure — a column headed "finance charge", or an interest amount shown beside the invoice. Empty string when the row has none, which is most rows. It is part of the amount, not extra to it: do not subtract it. Recording it lets the reconciliation say a difference is interest rather than a mis-keyed invoice.
...
document_pages: a supplier sending a month-end reprint may put a dozen SEPARATE INVOICES in one PDF — different invoice numbers, each with its own header and its own total. When that is what you are looking at, list one entry per invoice with the pages it occupies, first_page and last_page counting from 1 and both inclusive, covering every page of the file in order. Return an empty array for the ordinary case of one document, whatever else is stapled behind it: a packing slip, a delivery receipt or a second copy of the SAME invoice is not another invoice, and a statement listing many invoice numbers is one document, not many. Getting this wrong in either direction is expensive — a bundle read as one invoice loses every invoice but one, and a single invoice split apart invents bills that do not exist. When the array is non-empty, fill the other fields for the FIRST invoice only; each one is read separately afterwards.
...
function parseMoneyToCents(raw) {
  ...
  // Trailing minus: how tills and older accounting systems print a return.
  // Young's Home Building Centre puts a returned item on the invoice as
  // "60.12-", and read as a positive it turns a $60 bill into a $127 one.
...
// Do the line amounts, plus the tax, actually reach the printed total?
//
// The case this exists for: an Excel Business Systems invoice showed a
// "Contract copies charge $78.65" summary line AND then broke the same
// charge out beneath it as two meter readings ($55.47 + $23.18 = $78.65).
// Both levels were captured, so the bill posted to QuickBooks at exactly
// double — $177.74 against a document that said $88.87.
//
// A bill whose lines do not add up is wrong in a way nobody catches later:
// it looks plausible in QuickBooks, and it can never match the supplier's
// statement. Cheap to check, so it is checked.
//
// toleranceCents allows for a supplier rounding its own tax by a cent.
function linesReconcile({ lines, taxCents, totalCents }, toleranceCents = 1) {
  const lineTotalCents = (lines || []).reduce(
    (sum, l) => sum + (Number(l.amount_cents) || 0), 0
  );

  // undefined and null mean different things here, and conflating them cost
  // a live afternoon: the approve route left tax_cents out of its SELECT, so
  // it arrived undefined, was read as zero tax, and refused every taxable
  // invoice by exactly its own tax — with a message quoting a figure the
  // reviewer could not see on screen.
  //
  // null is a real answer ("no tax on this document") and is checked as
  // zero. undefined means nobody supplied the value, which is a caller bug,
  // not a document that fails to add up: skip the check and say so rather
  // than blocking a correct invoice.
  if (taxCents === undefined) {
    console.warn('[ap-extract] linesReconcile called without taxCents — check the caller\'s SELECT');
    return { ok: true, checked: false, lineTotalCents, expectedCents: null, differenceCents: 0 };
  }

  // Nothing to check against. criticalFieldsMissing() is what flags an
  // invoice with no total; this function's job is only the arithmetic.
  if (totalCents === null || totalCents === undefined) {
    return { ok: true, checked: false, lineTotalCents, expectedCents: null, differenceCents: 0 };
  }
  if (!lines || lines.length === 0) {
    return { ok: true, checked: false, lineTotalCents: 0, expectedCents: totalCents, differenceCents: 0 };
  }

  const expectedCents  = lineTotalCents + (Number(taxCents) || 0);
  const differenceCents = expectedCents - totalCents;
...
function normalizeExtraction(parsed) {
  ...
  const statementLines = (parsed.statement_lines || []).map((l, i) => ({
    ...
    amount_cents:         parseMoneyToCents(l.amount),
    kind:                 l.kind || 'other',
    finance_charge_cents: parseMoneyToCents(l.finance_charge),
  }));
  ...
  return {
    ...
    document_pages:  normalizeDocumentPages(parsed.document_pages),
    notes: parsed.notes || null,
  };
}

/**
 * The page ranges of several invoices bundled into one PDF.
 *
 * Guarded rather than trusted: a hallucinated range would cut a bill in half
 * or invent one, so anything that is not a sane, ascending, non-overlapping
 * run of whole pages is thrown away and the file is treated as the single
 * document it was always assumed to be. One entry is not a bundle either.
 */
function normalizeDocumentPages(raw) {
  if (!Array.isArray(raw) || raw.length < 2) return [];

  const out = [];
  let previousEnd = 0;
  for (const d of raw) {
    const from = Number(d?.first_page);
    const to   = Number(d?.last_page);
    if (!Number.isInteger(from) || !Number.isInteger(to)) return [];
    if (from < 1 || to < from) return [];
    if (from <= previousEnd) return [];   // overlapping or out of order
    previousEnd = to;
    out.push({ doc_number: d.doc_number || null, first_page: from, last_page: to });
  }
  return out;
}
...
async function extractDocument({ buffer, filename = 'document.pdf' }) {
  ...
  // Best effort: a file we cannot count pages in is still perfectly readable,
  // and one that is not a PDF at all has no page count to state.
  let pages = null;
  try {
    ...
    messages: [{
      ...
      content: [
        ...
        },
        {
          type: 'text',
          text:
            `Extract this supplier document. The file is named "${filename}", which may or may not be meaningful — trust the page, not the filename.` +
            // Without the count the model has to work out how long the file is
            // before it can say which pages each invoice occupies, and a
            // seven-page reprint pack came back as three ranges covering four
            // pages. Stating it turns "find the boundaries" into the far
            // easier "account for these N pages".
            (pages ? ` This PDF has ${pages} page${pages === 1 ? '' : 's'}. If it holds several separate invoices, every one of those ${pages} pages must appear in exactly one document_pages entry.` : ''),
        },
      ],
    }],
...
module.exports = {
  ...
  _internals: { DocumentSchema, SYSTEM, normalizeDocumentPages },
};

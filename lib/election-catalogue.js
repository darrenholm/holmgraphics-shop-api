...
function priceSigns({ cutKey, sheetKey, quantity, stands }) {
  ...
  const standsTotal = stands && cut.stands ? round2(pieces * WIRE_STAND) : 0;

  return {
    item_type: 'CoroplastSign',
    description:
      `${cut.name} coroplast sign, ${sheetLabel(sheetKey)}` +
      (standsTotal > 0 ? ', with wire stands' : '') +
...
function priceDecals({ widthIn, heightIn, quantity }) {
  ...
  return {
    item_type: 'Printing',
    description:
      `Vinyl decals ${w}" x ${h}" — ${across} across, ${rows} ${rows === 1 ? 'row' : 'rows'}, ` +
      `${round2(squareFeet)} sq ft of 54" roll` +

...
// OAuth token endpoint. Defaults to {host}/oauth/token where {host} is
// derived from SHIPTIME_BASE_URL (strip the /rest suffix). Override with
// SHIPTIME_OAUTH_URL if needed.
function defaultOauthUrl() {
  try {
    const u = new URL(SHIPTIME_BASE_URL);
    return `${u.protocol}//${u.host}/oauth/token`;
  } catch { return 'https://restapi.shiptime.com/oauth/token'; }
}

...
async function fetchOauthToken() {
  ...
  // TEMP DEBUG (remove once auth confirmed working). Logs lengths and
  // first/last 4 chars only — never the full secret.
  const _peek = (s) => s ? `len=${s.length} ${s.slice(0,4)}…${s.slice(-4)}` : '<empty>';

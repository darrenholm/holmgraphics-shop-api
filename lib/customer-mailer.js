...
// Staff-to-staff: a message about a job, sent to the assigned production
// employee from the job page's "Email <assignee>" button. The sender's name
// comes from the logged-in staff member's JWT; reply-to is left as the shop
// default so replies land in the ops inbox.
async function sendJobMessageToEmployee({ email, employeeName, projectId, projectTitle, message, senderName, jobUrl }) {
  const subject = `Message about job #${projectId}${projectTitle ? ': ' + projectTitle : ''}`;
  const fromLine = senderName ? `${senderName} sent you a message` : 'You have a message';
  return send({
    to: email,
    subject,
    kind: 'employee-job-message',
    text:
`${fromLine} about job #${projectId}${projectTitle ? ` (${projectTitle})` : ''}:

${message}

View the job: ${jobUrl}

-- Holm Graphics job dashboard`,
    html: wrap({
      heading: `Job #${escape(String(projectId))}${projectTitle ? ': ' + escape(projectTitle) : ''}`,
      bodyHtml:
        `<p style="margin:0 0 12px;">${escape(fromLine)}${employeeName ? ', ' + escape(employeeName.split(' ')[0]) : ''}:</p>
         <blockquote style="margin:0 0 16px;padding:10px 14px;border-left:3px solid #2b6cb0;background:#f7fafc;color:#1a1a1a;white-space:pre-wrap;">${escape(message)}</blockquote>`,
      ctaUrl: jobUrl,
      ctaLabel: `Open job #${projectId}`,
    }),
  });
}

// ─── Major defect found on a daily inspection ────────────────────────────────
// Fires once, at the moment a circle check completes with at least one major
// defect. That inspection is frozen the instant it completes, so this can't
// double-fire and needs no email_log idempotency row (email_log is keyed on
// order_id anyway, and there's no order here).
//
// This one is deliberately loud. A major defect means the unit is out of
// service under O. Reg. 199/07 — it does not move until an admin records a
// repair. Recipients come from FLEET_ALERT_TO, falling back to SHOP_QUOTES_TO
// and then to the From: address so a misconfigured env can never swallow it.
async function sendInspectionOutOfServiceAlert({ inspection, defects }) {
  const fromAddr = (SHOP_FROM.match(/<([^>]+)>/) || [, SHOP_FROM])[1];
  const recipients = (process.env.FLEET_ALERT_TO || process.env.SHOP_QUOTES_TO || fromAddr)
    .split(/[,;\s]+/).map(r => r.trim()).filter(Boolean);
  if (recipients.length === 0) return { sent: false, reason: 'no_recipients' };

  const unit     = inspection.unit_number || `vehicle ${inspection.vehicle_id}`;
  const reportUrl = `${PUBLIC_BASE}/fleet/check/${inspection.id}`;
  const subject  = `DO NOT OPERATE — ${unit} (${inspection.plate}) failed its circle check`;

  const lines = (defects || []).map(d =>
    `  • ${d.group_name} — ${d.item_label}\n    ${d.major_defect_text || 'Major defect'}${d.note ? `\n    Driver note: ${d.note}` : ''}`
  );

  const text = `MAJOR DEFECT — UNIT OUT OF SERVICE

Unit:       ${unit}  (${inspection.plate} ${inspection.plate_jurisdiction})
Inspector:  ${inspection.inspector_name}
Completed:  ${new Date(inspection.completed_at).toLocaleString('en-CA', { timeZone: 'America/Toronto' })}
Odometer:   ${inspection.odometer_km != null ? `${inspection.odometer_km} km (${inspection.odometer_source})` : '—'}
Location:   ${inspection.location_text || '—'}

Major defects found:
${lines.join('\n') || '  (none listed)'}

This unit may not be operated until the defect is repaired and an admin
records the repair against the inspection.

Report: ${reportUrl}
`;

  const html = wrap({
    heading: `DO NOT OPERATE — ${escape(unit)}`,
    bodyHtml:
      `<p style="margin:0 0 14px;padding:10px 12px;background:#fdecec;border-left:4px solid #c53030;color:#742a2a;font-size:14px;">
         <strong>Major defect recorded.</strong> This unit is out of service under
         O.&nbsp;Reg.&nbsp;199/07 and may not be operated until the defect is repaired
         and the repair is recorded.
       </p>
       <table cellpadding="6" cellspacing="0" style="font-size:14px;color:#1a1a1a;border-collapse:collapse;margin-bottom:14px;">
         <tr><td style="color:#718096;width:90px;">Unit</td><td><strong>${escape(unit)}</strong> &middot; ${escape(inspection.plate)} ${escape(inspection.plate_jurisdiction)}</td></tr>
         <tr><td style="color:#718096;">Inspector</td><td>${escape(inspection.inspector_name)}</td></tr>
         <tr><td style="color:#718096;">Odometer</td><td>${inspection.odometer_km != null ? escape(`${inspection.odometer_km} km (${inspection.odometer_source})`) : '&mdash;'}</td></tr>
         <tr><td style="color:#718096;">Location</td><td>${escape(inspection.location_text || '—')}</td></tr>
       </table>
       <h3 style="margin:0 0 6px;font-size:14px;color:#1a1a1a;">Major defects</h3>
       <ul style="margin:0 0 14px;padding-left:18px;font-size:14px;color:#1a1a1a;">
         ${(defects || []).map(d =>
           `<li style="margin-bottom:8px;"><strong>${escape(d.group_name)} — ${escape(d.item_label)}</strong><br>
              <span style="color:#4a5568;">${escape(d.major_defect_text || 'Major defect')}</span>
              ${d.note ? `<br><span style="color:#718096;">Driver note: ${escape(d.note)}</span>` : ''}</li>`
  ...
  const results = await Promise.all(
    recipients.map(to => send({ to, subject, html, text, kind: 'inspection-out-of-service' }))
  );
  const anyOk = results.some(r => r && r.ok);
  return anyOk
    ? { sent: true, recipients: recipients.length }
    : { sent: false, reason: 'send_failed', error: results.find(r => !r || !r.ok)?.error || 'unknown' };
}

// ─── Fleet digests (lib/inspection-jobs.js) ──────────────────────────────────
// Both go to FLEET_ALERT_TO, falling back to SHOP_QUOTES_TO then the From:
// address. Neither is sent when there is nothing to report — a digest that
// arrives every morning saying "all good" stops being read, and then the one
// that matters is not read either. The caller decides that; these just send.
function fleetRecipients() {
  const fromAddr = (SHOP_FROM.match(/<([^>]+)>/) || [, SHOP_FROM])[1];
  return (process.env.FLEET_ALERT_TO || process.env.SHOP_QUOTES_TO || fromAddr)
    .split(/[,;\s]+/).map(r => r.trim()).filter(Boolean);
}

async function sendInspectionDailyDigest({ needing, outOfService, date }) {
  const recipients = fleetRecipients();
  if (recipients.length === 0) return { sent: false, reason: 'no_recipients' };

  const boardUrl = `${PUBLIC_BASE}/fleet-admin/inspections`;
  const withWork = (needing || []).filter(u => u.has_work_today);
  const subject = outOfService?.length
    ? `Fleet: ${outOfService.length} unit${outOfService.length === 1 ? '' : 's'} out of service`
    : `Fleet: ${needing.length} unit${needing.length === 1 ? '' : 's'} without a valid circle check`;

  const line = (u) =>
    `  • ${u.unit_number} (${u.license_plate || 'no plate'})`
    + `${u.has_work_today ? '  — WORK BOOKED TODAY' : ''}\n`
    + `    ${u.last_completed_at
          ? `last checked ${new Date(u.last_completed_at).toLocaleString('en-CA', { timeZone: 'America/Toronto' })} by ${u.inspector_name}`
          : 'never checked'}`;

  const text = `Daily inspection status — ${date}

${outOfService?.length ? `OUT OF SERVICE
${outOfService.map(u => `  • ${u.unit_number} — ${u.open_major_defects} major defect(s) open`).join('\n')}

` : ''}${needing.length ? `No valid inspection${withWork.length ? ` (${withWork.length} with work booked today)` : ''}:
${needing.map(line).join('\n')}
` : 'Every in-scope unit has a valid inspection.\n'}
Board: ${boardUrl}
`;

  const html = wrap({
    heading: `Inspection status — ${escape(date)}`,
    bodyHtml:
      (outOfService?.length
        ? `<p style="margin:0 0 14px;padding:10px 12px;background:#fdecec;border-left:4px solid #c53030;color:#742a2a;font-size:14px;">
             <strong>Out of service:</strong>
             ${outOfService.map(u => `${escape(u.unit_number)} (${u.open_major_defects} major)`).join(', ')}
           </p>`
        : '')
      + (needing.length
        ? `<h3 style="margin:0 0 6px;font-size:14px;color:#1a1a1a;">No valid inspection</h3>
           <ul style="margin:0 0 14px;padding-left:18px;font-size:14px;color:#1a1a1a;">
             ${needing.map(u =>
               `<li style="margin-bottom:6px;">
                  <strong>${escape(u.unit_number)}</strong>
                  ${u.has_work_today ? '<span style="color:#c53030;font-weight:600;"> — work booked today</span>' : ''}
                  <br><span style="color:#718096;">${u.last_completed_at
                    ? `last checked ${escape(new Date(u.last_completed_at).toLocaleString('en-CA', { timeZone: 'America/Toronto' }))}`
                    : 'never checked'}</span>
                </li>`).join('')}
           </ul>`
        : `<p style="font-size:14px;color:#1f6b34;">Every in-scope unit has a valid inspection.</p>`),
    ctaUrl:   boardUrl,
    ctaLabel: 'Open the inspection board',
  });

  const results = await Promise.all(
    recipients.map(to => send({ to, subject, html, text, kind: 'inspection-daily-digest' }))
  );
  const anyOk = results.some(r => r && r.ok);
  return anyOk ? { sent: true, recipients: recipients.length } : { sent: false, reason: 'send_failed' };
}

async function sendFleetDocumentExpiryDigest({ expiring, missing }) {
  const recipients = fleetRecipients();
  if (recipients.length === 0) return { sent: false, reason: 'no_recipients' };

  const adminUrl = `${PUBLIC_BASE}/fleet-admin/vehicles`;
  const gone = (expiring || []).filter(d => d.days_remaining < 0);
  const subject = gone.length
    ? `Fleet documents: ${gone.length} expired`
    : `Fleet documents: ${expiring.length} expiring within 30 days`;

  const expLine = (d) => `  • ${d.unit_number} ${d.doc_type} — `
    + (d.days_remaining < 0
        ? `EXPIRED ${Math.abs(d.days_remaining)} day(s) ago (${d.expiry_date})`
        : `expires in ${d.days_remaining} day(s) (${d.expiry_date})`);

  const text = `Fleet document status

${expiring.length ? `Expiring or expired:
${expiring.map(expLine).join('\n')}
` : ''}${missing.length ? `
No current document on file:
${missing.map(m => `  • ${m.unit_number} — ${m.doc_type}${m.inspection_required ? '  (unit is in scope for daily inspections)' : ''}`).join('\n')}
` : ''}
A lapsed document does not stop a driver from recording a check — it is
recorded on the report as a warning instead. Fixing it is an office job.

Vehicles: ${adminUrl}
`;

  const html = wrap({
    heading: 'Fleet document status',
    bodyHtml:
      (expiring.length
        ? `<h3 style="margin:0 0 6px;font-size:14px;">Expiring or expired</h3>
           <ul style="margin:0 0 14px;padding-left:18px;font-size:14px;">
             ${expiring.map(d =>
               `<li style="margin-bottom:5px;color:${d.days_remaining < 0 ? '#a10000' : '#6c5300'};">
                  <strong>${escape(d.unit_number)}</strong> ${escape(d.doc_type)} —
                  ${d.days_remaining < 0
                    ? `expired ${Math.abs(d.days_remaining)} day(s) ago`
                    : `expires in ${d.days_remaining} day(s)`}
                  <span style="color:#718096;">(${escape(String(d.expiry_date))})</span>
                </li>`).join('')}
           </ul>`
        : '')
      + (missing.length
        ? `<h3 style="margin:0 0 6px;font-size:14px;">No current document on file</h3>
           <ul style="margin:0 0 14px;padding-left:18px;font-size:14px;">
             ${missing.map(m =>
               `<li style="margin-bottom:5px;"><strong>${escape(m.unit_number)}</strong> — ${escape(m.doc_type)}
                  ${m.inspection_required ? '<span style="color:#a10000;"> (in scope for daily inspections)</span>' : ''}</li>`
             ).join('')}
           </ul>`
        : '')
      + `<p style="font-size:13px;color:#718096;">A lapsed document does not stop a driver recording a
           check — it goes on the report as a warning instead. Fixing it is an office job.</p>`,
    ctaUrl:   adminUrl,
    ctaLabel: 'Open vehicles',
  });

  const results = await Promise.all(
    recipients.map(to => send({ to, subject, html, text, kind: 'fleet-expiry-digest' }))
  );
  const anyOk = results.some(r => r && r.ok);
  return anyOk ? { sent: true, recipients: recipients.length } : { sent: false, reason: 'send_failed' };
}
...
module.exports = {
  sendInspectionOutOfServiceAlert,
  sendInspectionDailyDigest,
  sendFleetDocumentExpiryDigest,
  sendJobQuote,
  sendJobMessageToEmployee,
  sendActivationEmail,

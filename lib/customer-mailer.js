...
async function sendProjectMessageNotification({ recipientEmail, audience, projectId, projectName, authorName, body }) {
  const fromAddr = (SHOP_FROM.match(/<([^>]+)>/) || [, SHOP_FROM])[1];
  const portalUrl   = audience === 'customer'
    ? `${PUBLIC_BASE}/portal/jobs/${projectId}`
    : `${PUBLIC_BASE}/jobs/${projectId}`;
  ...
  if (audience === 'customer') {
    ...
    const opsTo = process.env.SHOP_QUOTES_TO || fromAddr;
    String(opsTo).split(/[,;]/).map((s) => s.trim()).filter(Boolean).forEach((e) => to.push(e));
...
-- Holm Graphics`,
    html: wrap({
      ...
      ctaUrl:   portalUrl,
      ctaLabel: audience === 'customer' ? 'View thread' : 'Reply in the job board',
    }),
  });
}

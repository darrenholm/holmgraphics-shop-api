...
async function send({ to, subject, html, text, kind, replyTo }) {
...
async function sendProjectProofRequest({ recipientEmail, projectId, projectName, version, approvalUrl, imageUrl, senderName, note }) {
  const fromAddr = (SHOP_FROM.match(/<([^>]+)>/) || [, SHOP_FROM])[1];
  ...
  // pick the first one for the customer's Reply button. They'll still hit
  // a human who can fan it out internally.
  const staffInbox = process.env.SHOP_QUOTES_TO || fromAddr;
  const replyToOne = String(staffInbox).split(/[,;]+/).map((s) => s.trim()).filter(Boolean)[0] || fromAddr;
...
async function sendProjectProofResponseToStaff({ recipientEmail, projectId, projectName, version, kind, customerName, text, annotationsCount, jobUrl }) {
  const fromAddr = (SHOP_FROM.match(/<([^>]+)>/) || [, SHOP_FROM])[1];
  const staffInbox = process.env.SHOP_QUOTES_TO || fromAddr;
  const to = [];
  if (recipientEmail) to.push(recipientEmail);
  String(staffInbox).split(/[,;]/).map((s) => s.trim()).filter(Boolean).forEach((e) => {
    if (!to.some((x) => x.toLowerCase() === e.toLowerCase())) to.push(e);
  });
...
${text ? `Customer note:\n"${text}"\n\n` : ''}${annotationsCount ? `Customer added ${annotationsCount} annotation${annotationsCount === 1 ? '' : 's'} on the proof — view them in the job's Proofs tab.\n\n` : ''}Open the job: ${jobUrl}
    ...
    html: wrap({
      ...
      bodyHtml:
        `<p style="margin:0 0 12px;color:#1a1a1a;">
         ...
         ${annotationsCount ? `<p style="margin:0 0 12px;color:#4a5568;font-size:13px;">Customer added <strong>${annotationsCount}</strong> annotation${annotationsCount === 1 ? '' : 's'} on the proof. Open the job's Proofs tab to view them.</p>` : ''}`,

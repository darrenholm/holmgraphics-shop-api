 ...
 */
async function sendProjectMessageNotification({ recipientEmail, audience, projectId, projectName, authorName, body }) {
  const fromAddr = (SHOP_FROM.match(/<([^>]+)>/) || [, SHOP_FROM])[1];
  ...
  // Customer replies should land in the staff inbox (not the no-reply From),
  // and the CTA should make plain that they reply via email — most customers
  // don't have an activated portal account and the "View thread" link was
  // sending them to a login wall.
  const replyTo = audience === 'customer' ? staffInbox : undefined;

...
async function sendStaffNewOrderAlert({ orderId, db }) {
  ...
  try {
    order = await db.queryOne(
      `SELECT id, order_number, fulfillment_method, payment_method,
              grand_total, notification_email, project_id, client_id,
              created_at
  ...
  const itemsText = items.length
    ? items.map(i => `  ${i.quantity}× ${i.description || 'Item'} — ${money(i.line_total)}`).join('\n')
    : '  (no items)';
  const itemsHtml = items.length
    ? '<ul style="margin:0 0 12px;padding-left:18px;">' +
      items.map(i =>
        `<li><strong>${escape(String(i.quantity))}×</strong> ${escape(i.description || 'Item')} — ${escape(money(i.line_total))}</li>`
      ).join('') +
      '</ul>'

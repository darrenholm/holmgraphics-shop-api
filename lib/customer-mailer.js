...
async function sendProjectMessageNotification({ recipientEmail, audience, projectId, projectName, authorName, body, replyToEmail }) {
  ...
  // Customer replies route to the assigned staff member if we have their
  // email; otherwise to the shared ops inbox; otherwise the From address.
  // The CTA tells the customer to just reply to the email — most don't
  // have an activated portal account so the old "View thread" link was a
  // login wall.
  const replyTo = audience === 'customer'
    ? (replyToEmail || staffInbox)
    : undefined;
